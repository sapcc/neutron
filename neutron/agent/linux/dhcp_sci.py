...
class ProcessWrapper:
    ...
    def _spawn_or_reload(self, reload_with_HUP):
        ...
        # not successful.
        if pm.active and self.reload(allocations_only=True):
    ...
    def reload(self, allocations_only=False) -> bool:
        """Called when the process should reload its configuration.
           ...
           Allocations_only is True when the process should only reload
           host allocations / IP adress mappings and not the whole config.
           E.g. to preserve the DNS cache.
           When this method returns False, we fallback to upstream code
           which uses an external command or SIGHUP to reload, etc.
           This also means returning False here will try an alternative method
           when we get an error during reload.
        """
        return False
...
class WrapUnbound(ProcessWrapper):
    ...
    def reload(self, allocations_only: bool = False) -> bool:
    ...
    def reload_rpz(self):
        """Reload RPZ only, keeping cache and statistics
        """
        try:
            ret = self._send_unbound_ctrl(
                    f'auth_zone_reload {self._rpz_name()}'
            )
            if ret:
                ret = ret.decode().strip()
            if ret == 'ok':
    ...
    @property
    def has_dns_forwarders(self) -> bool:
        """returns if forwarders are configured. Used to optimize unbound
        config for local recursion or forwarding.
        """
        if self._has_dns_forwarders is None:
            self._has_dns_forwarders = bool(self._get_net_dns_upstreams())
        return self._has_dns_forwarders
    ...
    def unbound_metrics(self, columns=None, prefixlist=None, reset=False)\
        ...
        try:
            buf = self._send_unbound_ctrl(cmd, limit=4096)
        except OSError as e:
    ...
    def _rpz_name(self):
        # We are free to choose a value, but we would like to keep it
        # inside the local dns tree and avoid collisions with existing
        # domain names.
        if self.conf.dns_domain:
            dns_domain = self.conf.dns_domain.strip('.')
            return f"{self.network.id}.rpz.{dns_domain}."
        return f"{self.network.id}.rpz.invalid."
    ...
    def _output_unbound_config_file(self):
        ...
        if self.has_dns_forwarders:
            # when upstreams are configured, we want to forward queries
            # for private and special use prefixes, referred to as AS112
            # by unbound. Also we do not want to check DNSSEC for those PTR
            settings['unblock-lan-zones'] = 'yes'
            settings['insecure-lan-zones'] = 'yes'

            # time to live for entries in the host cache incl roundtrip timing
            # this is important when our upstreams are briefly unavailable.
            # they are in our own infra, so we assume they usually are up
            # again quickly
            settings['infra-host-ttl'] = '10'
            # also keep probing hosts that were marked down
            settings['infra-keep-probing'] = 'yes'
        else:
            # default is 900
            settings['infra-host-ttl'] = '120'
    ...
    def _unbound_cfg_add_dnstap(self, buf):
        ...
        # only add dnstap config if enabled
        if dnstap_enabled:
            ...
            identity = f"{netid}.{self.network.project_id}.{agent}"

            if suffix := self.conf.SCI.dnstap_suffix:
                identity += f".{suffix}"
    ...
    def _unbound_cfg_add_upstreams(self, buf):
        ...
        # when forwarders are set, we also forward all as112 zones, see the
...
class UnboundDnsmasq(DhcpLocalProcess):
    ...
    def disable(self, retain_port=False, block=False, **kwargs):
        """Disable DHCP for this network by killing the local processes.
           We stop unbound first, then invoke it on dnsmasq which also
           does some cleaning.
        """
