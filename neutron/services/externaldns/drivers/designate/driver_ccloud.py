...
class DesignateCcloud(driver.ExternalDNSService):
    ...
    def _list_all_pages(function, criterion, args=None):
        ...
        default) in a single response, so in practice one request is enough
        for the narrowly filtered lookups done here. The 'next' link is still
    ...
    def _zone_usable_by_caller(self, user_client, zone, context):
    ...
    def _prune_recordsets(self, designate_client, zone_ref, recordsets,
        ...
        Returns False when a change was refused, so that the caller can retry
        the cleanup with stronger credentials.
        """
        complete = True
        for rs in recordsets:
            rs_type = rs.get('type')
            if rs_type and rs_type not in ('A', 'AAAA'):
                continue
            ...
            except d_exc.NotFound as exc:
                LOG.debug("Designate record %(recid)s in %(zone)s is already "
                          "gone: %(err)s",
                          {'recid': rs['id'], 'zone': zone_ref, 'err': exc})
            except (d_exc.Forbidden, d_exc.BadRequest) as exc:
                LOG.debug("Caller may not change Designate record %(recid)s "
                          "in %(zone)s: %(err)s",
                          {'recid': rs['id'], 'zone': zone_ref, 'err': exc})
                complete = False
        return complete
    ...
    def delete_record_set(self, context, dns_domain, dns_name, records):
        client, _admin_client = get_clients(context)
        ...
        try:
            ...
            if not self._prune_recordsets(client, dns_domain, recordsets,
    ...
    def _delete_in_scoped_zone(self, context, dns_domain, fqdn, records):
        ...
        if not context.is_admin:
            ...
            zones_to_clean = [
                ...
                if self._zone_usable_by_caller(user_client, zone, context)]
        ...
        for zone in zones_to_clean:
            zone_id = zone['id']
            if zone.get('project_id') != context.project_id:
                LOG.info("Cross-project DNS cleanup of '%(fqdn)s' %(recs)s "
                         "in zone %(zone)s owned by project %(owner)s, "
                         "requested by user %(user)s of project %(proj)s",
                         {'fqdn': fqdn, 'recs': records, 'zone': zone_id,
                          'owner': zone.get('project_id'),
                          'user': getattr(context, 'user_id', None),
                          'proj': context.project_id})
            try:
